...
- `IDLE`
  - No active timed workflow is running.
  - `paused` must be `false`.
- `MAPPING`
  - Active workflow phase 1.
  - Duration is 5 seconds of unpaused time.
- `CONSTRUCTING_ROUTE`
...
4. After 5 seconds of unpaused time, server transitions to `CONSTRUCTING_ROUTE`, `paused=false`
5. After another 5 seconds of unpaused time, server transitions to `IDLE`, `paused=false`

Pause behavior:

- `pause` is only valid while the workflow is active, meaning state is not `IDLE`.
- While paused, the active phase timer stops advancing.
- `resume` continues the current phase timer from the remaining time.
...
- Only one timed workflow should run at a time.
- `start_mapping` should be rejected if the server is not currently `IDLE`.
- `pause` and `resume` should affect elapsed timer progress, not just UI state.
